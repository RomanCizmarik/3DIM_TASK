...
QDicomImage::QDicomImage(const QString& filename, const unsigned long flags,
                         const unsigned long fstart, const unsigned long fcount)
    : d(new QDicomImagePrivate())
{
    d->m_dicomFileFormat = new DcmFileFormat();

    loadDICOMImage(filename,flags,fstart,fcount);
}
...
void QDicomImage::loadDICOMImage(const QString &filename, const unsigned long flags,
                                 const unsigned long fstart, const unsigned long fcount)
{
    d->m_succes = false;
    //    d->m_dicomFileFormat = new DcmFileFormat();
    OFCondition cond = d->m_dicomFileFormat->loadFile(filename.toStdString().c_str(), EXS_Unknown,
    ...
    if(cond.good())
    {
        ...
        // if image already exists, delete it before creating new one
        if(d->m_dicomImage != NULL){
            delete d->m_dicomImage;
            d->m_dicomImage = NULL;
        }
...
QString QDicomImage::getPatientName()
{
    return patientName;
}

QString QDicomImage::getInstitutionName()
{
    return institutionName;
}

QString QDicomImage::getStudyDate()
{
    return studyDate;
}


void QDicomImage::setPatientName()
{

    OFString patientNameOFS;
    if(d->m_dicomFileFormat->getDataset()
            ->findAndGetOFString(DCM_PatientName,patientNameOFS).good()){
        patientName = patientNameOFS.c_str();

    }
    else{
        patientName = "";
        qDebug()<<"Patient Name not found";
    }
}

void QDicomImage::setAcquisitionDateTime()
{
    OFString StudyDateOFS;
    if(d->m_dicomFileFormat->getDataset()
            ->findAndGetOFString(DCM_StudyDate,StudyDateOFS).good()){
        studyDate = StudyDateOFS.c_str();

    }
    else{
        studyDate = "";
        qDebug()<<"acquisitionDateTime not found";
    }
}

void QDicomImage::setInstitutionName()
{

    OFString InstitutionNameOFS;
    if(d->m_dicomFileFormat->getDataset()
            ->findAndGetOFString(DCM_InstitutionName,InstitutionNameOFS).good()){
        institutionName = InstitutionNameOFS.c_str();
